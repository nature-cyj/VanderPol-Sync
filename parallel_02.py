...
def initial_two_metronome(initial):
	# Initial condition
...
color = list(map(initial_two_metronome, initial_list))

x1_0s, x2_0s = initial_list[:,0], initial_list[:,2]
f = open('par d{:.2f} b{:.2f} mu{:.2f} x0{:.2f}.py'.format(d,b,mu,x0),'w')
